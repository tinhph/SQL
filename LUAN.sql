...
INSERT INTO `queryexercises`.`products` (`productId`, `typeProduct`,`productName`, `price`, `rating`) VALUES ('8', 'electronic','IMac', 10000, 2);
INSERT INTO `queryexercises`.`products` (`productId`, `typeProduct`,`productName`, `price`, `rating`) VALUES ('9', 'electronic','Printer', 4500, 1);
INSERT INTO `queryexercises`.`products` (`productId`, `typeProduct`,`productName`, `price`, `rating`) VALUES ('10', 'food','coca', 200, 1);
...
/* 5. FAIL (totalPrice is 9600 and totalQuantity is 8) Hiển thị productName, totalPrice, quanlity các đơn hàng đã order có productName là IphoneX--*/
select productName, sum(price) as totalPrice, quantity
from orders
inner join products
on products.productId = orders.productId
...
/*6. FAIL (totalPrice of DVD is 1000 and totalQuantiti is 5) Hiển thị productName, totalPrice, quanlity các đơn hàng đã order có typeProduct là electronic--*/
select productName, sum(price) as totalPrice, quantity
from products
inner join orders
...
/*7. FAIL (products.price * orders.quantity) as totalPrice Hiển thị chi tiết đơn hàng và tổng tiền cho mỗi mặt hàng--*/
select orders.orderId, productName, typeProduct, sum(price) as totalPrice
from orders
inner join products
on products.productId = orders.productId
group by productName;
...
/*8. FAIL (totalPrice not exactly) Hiển thị danh sách số lượng mặt hàng đã order, tổng tiền mặt hàng đã được order theo từng loại*/
select count(orders.productId) as totalProduct, typeProduct, sum(price) as totalPrice
from orders
inner join products
on products.productId = orders.productId
group by typeProduct;
...
/*9.FAIL Hiển thị danh sách tổng số mặt hàng, tổng số tiền của mặt hàng đó theo typeProduct được order trong tháng 1*/

select count(orders.productId) as totalProduct,sum(price) as totalPrice, typeProduct
from orders
inner join products
on orders.productId = products.productId
...
select count(orders.productId ) as max, productName
from products
inner join orders
on orders.productId = products.productId
group by productName;
...
/*11. FAIL (totalQuantity of electronic is 10) Hiển thị các mặt hàng đã được order trong khoảng tháng 2 đến tháng 3--*/
select count(orders.productId) as totalProduct, typeProduct
from products
inner join orders
on products.productId = orders.productId
